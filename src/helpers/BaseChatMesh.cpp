...
void BaseChatMesh::onAdvertRecv(mesh::Packet* packet, const mesh::Identity& id, uint32_t timestamp, const uint8_t* app_data, size_t app_data_len) {
  ...
  if (from == NULL) {
    if (!shouldAutoAddContactType(parser.getType())) {
      ...
      onDiscoveredContact(ci, true, packet->path_len, packet->path, packet->getSNR());       // let UI know
      return;
    }
    ...
    if (from == NULL) {
      ContactInfo ci;
      populateContactFromAdvert(ci, id, parser, timestamp);
      onDiscoveredContact(ci, true, packet->path_len, packet->path, packet->getSNR());
      onContactsFull();
  ...
  onDiscoveredContact(*from, is_new, packet->path_len, packet->path, packet->getSNR());       // let UI know
}
